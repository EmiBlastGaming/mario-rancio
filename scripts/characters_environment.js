...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Presiona el boton de iniciar para, pues ya sabes ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
  ...
  if(gameConfig.status==='gameover'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("PERDISTE JAJA", gameConfig.screenX/2, gameConfig.screenY/2+105);
    textSize(15);
    text("Refreshea la website para volver a empezar", gameConfig.screenX/2, gameConfig.screenY/2+135);
    textSize(40);
...
function manualControl(character){
  ...
  if(character.live){
    ...
    if(!keyDown(control.left)&&!keyDown(control.right)&&!keyDown(control.up)){ 
      // haracter.changeAnimation('stand');
    }
  }
...
function scores(character){
  ...
  text("puntuacion: "+round(gameConfig.scores),20,40);
